...
local function get_formspec(structure)
    ...
    formspec = formspec .. "label[0.5,3.6.1;Usage Priority]"
...
local function is_net_low_resources(net)
    ...
    return net_mass < 0.07 or net_energy < 0.10
end

local function is_net_not_low_resources(net)
    local net_mass = net.mass_storage > 0 and net.mass / net.mass_storage or 0
    local net_energy = net.energy_storage > 0 and net.energy / net.energy_storage or 0
    return (net_mass >= 0.07 or net_energy >= 0.10) and net_mass > 0.03 and net_energy > 0.03
end

local function is_net_has_resources(net)
    local net_mass = net.mass_storage > 0 and net.mass / net.mass_storage or 0
    local net_energy = net.energy_storage > 0 and net.energy / net.energy_storage or 0
    return (net_mass > 0.5 or net_energy > 0.7) and net_mass > 0.03 and net_energy > 0.03
end

local function is_net_not_has_resources(net)
    local net_mass = net.mass_storage > 0 and net.mass / net.mass_storage or 0
    local net_energy = net.energy_storage > 0 and net.energy / net.energy_storage or 0
    return (net_mass < 0.3 or net_energy < 0.5) and net_mass < 0.5 and net_energy < 0.6
end

local function is_net_full_resources(net)
    local net_mass = net.mass_storage > 0 and net.mass / net.mass_storage or 0
    local net_energy = net.energy_storage > 0 and net.energy / net.energy_storage or 0
    return (net_mass > 0.98 or net_energy > 0.97) and net_mass > 0.47 and net_energy > 0.67
end
...
local function find_target(structure, net)
    ...
    if is_net_has_resources(net) then
        structure._is_reclaiming = false
    end
    if _do_reclaim and is_net_low_resources(net) then
    ...
    if build_focus >= 1 and build_focus <= 4 then
        if _do_assist or _do_repair then
            if is_net_not_low_resources(net) and not structure._is_reclaiming then
                --core.log("find_build_target")
                -- find build target
                find_build_target(structure)
            end
        end
        if structure._build_target == nil and _do_reclaim then
...
local function do_focus_build(s_obj, b_power, t_structure, t_unit, t_factory, net)
    ...
    if is_net_not_has_resources(net) then
        return false
    end
        ...
        if b_found and s_obj._build_target ~= nil then
            ...
            if s_obj._target_locked and has_structure_or_unit then
                ...
                if net.energy - (1 + e_use) > 0 and net.mass - (1) > 0 then
                    local b_power = s_obj:get_data():get_build_power()
                    local b_priority = meta:get_int("build_priority")
                    if b_priority == 1 and is_net_not_has_resources(net) then
                        b_power = b_power * 0.5
                        if is_net_low_resources(net) then
                            b_power = b_power * 0.5
                        end
            ...
            if has_structure_or_unit and s_obj._build_target.reclaim ~= nil then
                if s_obj._out_index > 1 and is_net_low_resources(net) then
